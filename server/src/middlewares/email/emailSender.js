...
let transporter = nodemailer.createTransport({
  ...
  const createMessage = async (client) => {
    ...
    const closureDate = new Date(client.closure_date).toLocaleDateString("en-US", {
      year: 'numeric', month: 'long', day: 'numeric'
    });
  
    const today = new Date();
    const closureDate2 = new Date(client.closure_date);
    const monthsSinceClosure = today.getMonth() - closureDate2.getMonth() + 
  ...
  for (const month of checkMonths) {
    ...
    const clients = await Client.findAll({
      where: {
        status: 'closed',
        closure_date: {
          [Op.gte]: new Date(targetDate.getFullYear(), targetDate.getMonth(), targetDate.getDate(), 0, 0, 0),
          [Op.lt]: new Date(targetDate.getFullYear(), targetDate.getMonth(), targetDate.getDate() + 1, 0, 0, 0),
        }
      }
    });
