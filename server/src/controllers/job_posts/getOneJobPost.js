...
const getJobPostsRequestHandler = async (req, res) => {
    ...
    try{
        ...
        const requiredFields = ['id', 'title', 'employer', 'location', 'hours_per_week', 'rate_of_pay_min', 'rate_of_pay_max', 'rate_of_pay_frequency', 'job_type', 'close_date', 'job_description', 'custom_questions', 'creator', 'state'];

        // Get one Job Posts
        const jobPost = await JobPosting.findOne({where: {id: jobPostId}, attributes: requiredFields});
