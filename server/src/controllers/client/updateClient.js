...
const updateClientRequestHandler = async (req, res) => {
  try {
    ...
    if (!req.body.values) {
      return res.status(401).json({status: "fail", message: "Missing values to update in parameters", data: null});
    }

    await client.set(req.body.values);
    await client.save();

    return res.status(200).json({status: "success", message: "Client updated successfully", data: client});
